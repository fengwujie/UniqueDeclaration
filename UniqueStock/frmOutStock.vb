...
Public Class frmOutStock
    ...
    Private Sub LoadAndyData(Optional ByVal strOrder As String = "")
        ...
        If chk_noinstock.Checked Then
            strCnd &= " And (select sum(isnull(已入库数量,0)-isnull(已出库数量,0)) from 报关制造通知单明细表 where 报关制造通知单明细表.制造通知单id=报关制造通知单表.制造通知单id)>0"
        End If
    ...
    Private Sub gridView_SelectionChanged(ByVal sender As System.Object, ByVal e As EventArgs)
        ...
        If tdg_to.CurrentRow IsNot Nothing Then
            ...
            lblStockNo.Text = pdtReturn.Rows(0)("出库单号").ToString
    ...
    Private Sub LoadDetail(ByVal strFromId As String)
        ...
        Dim strSQL As String = "select 制造通知单明细表id,制造通知单id,客人型号,优丽型号,颜色,订单数量,(已入库数量-isnull(已出库数量,0)) As 库存数量,单位,已出库数量,0 As 出库数量,出库单价,Convert(varchar(10),出库日期,120) as 出库日期 from 报关制造通知单明细表 Where isnull(订单数量,0)>0 and 制造通知单id=" & strFromId
        ...
        For Each dtRow As DataRow In pdtReturn.Rows
            If dtRow("出库日期").ToString <> "" Then
                If DateTime.Parse(dtRow("出库日期").ToString) <= Date.Parse("1900/01/01") Then
                    dtRow("出库日期") = ""
                End If
            End If
        Next
        ...
        If pdtReturn IsNot Nothing Then
            ...
            tdgDetail.Columns(8).ReadOnly = True
            tdgDetail.Columns(8).DefaultCellStyle.BackColor = Color.Silver
    ...
    Private Sub btnExit_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnExit.Click
        If MessageBox.Show("确定要全部出库吗？", "提示", MessageBoxButtons.OKCancel) = Windows.Forms.DialogResult.Cancel Then
            Return
        End If
        ...
        For Each dtRow As DataRow In dtData.Rows
            ...
            If Val(dtRow("已出库数量").ToString) < Val(dtRow("库存数量").ToString) Then
                strSql = "Update 报关制造通知单明细表 set 已出库数量 =已入库数量,出库日期='" & DateTimePicker1.Value & "' where 制造通知单明细表id=" & dtRow("制造通知单明细表id").ToString
        ...
        If intUpCount > 0 Then
            ...
            If Me.lblStockNo.Text = "" Then
                strSql = "select max(出库单号) from 报关制造通知单表 where 出库单号 like '" & DateTime.Now.ToString("yyyyMMdd") & "%'"
                ...
                strSql = "update 报关制造通知单表 set 出库单号='" & strNo & "' Where 制造通知单id=" & mstrFromId
                Dim intR As Integer = objData.ExcelSql(strSql, strError)
                If intR < 0 Then
                    MessageBox.Show("更新出库单失败：" & strError)
                End If
            End If
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        For Each dtRow As DataRow In dtData.Rows
            ...
            If Val(dtRow("已出库数量").ToString) + Val(dtRow("出库数量").ToString) > Val(dtRow("库存数量").ToString) Then
                MessageBox.Show("优丽型号：" & dtRow("优丽型号").ToString & "  已出库数量 + 出库数量已超出库存数量，请修改！")
                strSql = "Update 报关制造通知单明细表 set 出库单价=" & Val(dtRow("出库单价").ToString) & ",出库日期='" & DateTimePicker1.Value & "' where 制造通知单明细表id=" & dtRow("制造通知单明细表id").ToString
                Dim intRet As Integer = objData.ExcelSql(strSql, strError)
                blnRef = True
            ElseIf Val(dtRow("出库数量").ToString) > 0 Then
                strSql = "Update 报关制造通知单明细表 set 已出库数量 =isnull(已出库数量,0)+" & dtRow("出库数量").ToString & ",出库单价=" & Val(dtRow("出库单价").ToString) & ",出库日期='" & DateTimePicker1.Value & "' where 制造通知单明细表id=" & dtRow("制造通知单明细表id").ToString
                Dim intRet As Integer = objData.ExcelSql(strSql, strError)
                If intRet < 0 Then
                    MessageBox.Show("更新错误：" & strError)
                Else
        ...
        If MsgBox("数据是否导入EXCEL文件？", vbYesNo + vbQuestion, "提示") = vbYes Then  ' 用户按下"是"。
            ...
            Try
                ...
                If dtmRs.Rows.Count > 0 Then
                    ...
                    objExcelWorkSheet.Range("A1").Value = "成品出库单"
                    ...
                    objExcelWorkSheet.Cells(n, 7) = "出库数量"
                    objExcelWorkSheet.Cells(n, 8) = "出库日期"
                    ...
                    For Each dtRow As DataRow In dtDetail.Rows
                        ...
                        objExcelWorkSheet.Cells(n, 7) = dtRow("已出库数量").ToString
                        objExcelWorkSheet.Cells(n, 8) = dtRow("出库日期").ToString.Split(" ")(0)
    ...
    Private Sub btnReset_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnReset.Click
        ...
        For Each dtRow As DataRow In dtData.Rows
            ...
            If Val(dtRow("已出库数量").ToString) > 0 Then
                strSql = "Update 报关制造通知单明细表 set 已出库数量 =0,出库日期='1900/01/01' where 制造通知单明细表id=" & dtRow("制造通知单明细表id").ToString
                Dim intRet As Integer = objData.ExcelSql(strSql, strError)
                If intRet < 0 Then
                    MessageBox.Show("更新错误：" & strError)
                Else
        ...
        If intUpCount > 0 Then
            ...
            If Me.lblStockNo.Text <> "" Then
                ...
                Me.lblStockNo.Text = ""
                strSql = "update 报关制造通知单表 set 出库单号='' Where 制造通知单id=" & mstrFromId
                Dim intR As Integer = objData.ExcelSql(strSql, strError)
                If intR < 0 Then
