...
Public Class frmInStockQuery
    ...
    Private Sub LoadAndyData(Optional ByVal strOrder As String = "")
        ...
        Dim strSQL As String = "select 料件入库表id,入库单号,Convert(varchar(10),入库时间,120) As 入库日期,摘要,报关单号,发票号,电子帐册号,录入员,过帐标志,清单编号 from 进口料件入库表 Where 1=1"
        Dim strCnd As String = ""

        If txtNo.Text.Trim <> "" Then
            strCnd &= " And 入库单号 like '%" & txtNo.Text & "%'"
        End If
        If txtCstNo.Text.Trim <> "" Then
            strCnd &= " And 报关单号 like '%" & txtCstNo.Text & "%'"
        End If
        If DateTimePicker1.Checked Then
            strCnd &= " And 入库时间 >= '" & DateTimePicker1.Value.ToShortDateString & "'"
        End If
        If DateTimePicker2.Checked Then
            strCnd &= " And 入库时间 <= '" & DateTimePicker2.Value.ToShortDateString & "'"
        End If
        ...
        If pdtReturn IsNot Nothing Then
            ...
            'Me.tdg_to.Columns(16).Visible = False
            'Me.tdg_to.Columns(15).Visible = False
            'Me.tdg_to.Columns(14).Visible = False
            'Me.tdg_to.Columns(13).Visible = False
            'Me.tdg_to.Columns(12).Visible = False
            'Me.tdg_to.Columns(11).Visible = False
            'Me.tdg_to.Columns(10).Visible = False
    ...
    Private Sub gridView_SelectionChanged(ByVal sender As System.Object, ByVal e As EventArgs)
        ...
        If tdg_to.CurrentRow IsNot Nothing Then
            Dim strFromId As String = tdg_to.CurrentRow.Cells("料件入库表id").Value.ToString()
            lblShowNo.Text = tdg_to.CurrentRow.Cells("入库单号").Value.ToString()
    ...
    Private Sub LoadDetail(ByVal strFromId As String)
        ...
        pdtReturn = objData.SqlProc4("进口料件入库录入查询", strFromId, lblShowNo2.Text, strError)
        If pdtReturn Is Nothing Then
            MessageBox.Show("查询出现错误：" & strError)
        End If
    ...
    Private Sub CmdExcel_Click()
        If MsgBox("数据是否导入EXCEL文件？", vbYesNo + vbQuestion, "提示") = vbYes Then  ' 用户按下"是"。
            ...
            Try
                ...
                Dim objData As New ErpDataHelper(My.Settings.server, My.Settings.datasource, My.Settings.sa, My.Settings.pw)
                Dim dtmRs As DataTable = objData.GetDataTableBySql("select * from 进口料件入库表 where 入库单号='" & lblShowNo.Text & "'", "")
                If dtmRs.Rows.Count > 0 Then
                    objExcelWorkBook = objExcelApp.Workbooks.Open(Application.StartupPath & "\excel\材料入库单.XLS", , True)
                    ...
                    For Each dtRow As DataRow In dtDetail.Rows
                        ...
                        objExcelWorkSheet.Cells(n, 4) = dtRow("入库数量").ToString.Trim
